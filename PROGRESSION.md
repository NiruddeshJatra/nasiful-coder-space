...
- **`src/articles/manifest.ts`**: added `enDescription`/`bnDescription`/`datePublished` fields to `ArticleEntry` and `INTRO_ARTICLE` — the single source of truth for article SEO metadata. Added `SERIES_DESCRIPTION_EN`/`_BN`.
